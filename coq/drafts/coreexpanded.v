...
Proof.
  ...
  induction n as [|n'].
    ...
    - intros. simpl. Set Printing All.
      generalize (@eq_sym (sliceannshape (S dim)) sas (@carve_sliceannshapedim_right (S dim) sas axis axis_lt_sdim)
        (@carve_sliceannshapedim_right_is_inc_sliceannshapedim_n_right_inverse dim sas axis O axis_lt_sdim
           sas_has_no_rpad)). unfold transport.
      generalize (@eq_sym (shape (S dim)) (@innershape (S dim) sas) (@innershape (S dim) sas)
              (@innershape_id_inc_sliceannshapedim_n (S dim) sas axis O axis_lt_sdim)).
      intro e. rewrite (UIP_refl _ _ e). clear e.
      simpl.
      induction axis as [|axis'].
        (* axis = O *)
        * simpl. dependent destruction sas. destruct s as [abeg aend pbeg pend].
          simpl. unfold carve_sliceanndimbound_right. simpl.
          generalize (carve_sliceanndimbound_right_obligation_2 bound (sadim bound abeg aend pbeg pend)).
          simpl. intro le_refl_. rewrite (proof_irrelevance _ le_refl_ (le_refl aend)). clear le_refl_.
          simpl. intro shape_id. dependent destruction shape_id.
          generalize dependent shape_id0.
          rewrite (proof_irrelevance _ (le_refl bound) pend).
          intros e _. rewrite (UIP_refl _ _ e). clear e. simpl.
          destruct saa as [v].
          f_equal. simpl.
          generalize dependent v.
          generalize (take_obligation_1 dim (scons (sliceanndimbound (sadim bound abeg bound pbeg pend)) (outershape sas)) bound pend).
          simpl. unfold sliceanndimbound.
          induction v as [|n' e v'].
            (* v = vnil *)
            + reflexivity...
            (* v = vcons e v' *)
            + simpl. f_equal.
              apply (IHv' (le_S_n n' n' l)).
        (* axis = 
          dependent destruction v.

          destruct v. v as [|v'].
          in shape_id0. generalize dependent shape_id0.
          simpl_one_JMeq. simpl.
          injection shape_id. inversion.
unfold inv_cpadr_all. generalize dependent saa.
      generalize dependent pi_gt_O.
      rewrite -> sas_has_no_rpad. simpl. rewrite (proof_irrelevance _ e eq_refl). clear e.
      simpl. (UIP_refl _ e).
      simpl. (eq_sym (innershape_id_inc_sliceannshapedim_n axis 0 axis_lt_sdim)).
...
Lemma inv_cpadr_all_is_cpadr_right_inverse :
  forall {dim} {sas : sliceannshape (S dim)} (saa : sliceannarray sas) (axis : nat) (n : nat)
         ...
         (pi_gt_O : 0 < pi (innershape sas)),
    transport saa
       (carve_sliceannshapedim_right_is_inc_sliceannshapedim_n_right_inverse sas axis n axis_lt_sdim sas_has_no_rpad)
    = inv_cpadr_all
       (cpadr_n saa axis n axis_lt_sdim pi_gt_O) axis axis_lt_sdim
          (transport (P:=fun x => 0 < pi x) pi_gt_O (eq_sym (innershape_id_inc_sliceannshapedim_n _ _ _))).
Proof.
  ...
  induction n as [|n'].
    (* n = O *)
    - intros. simpl. dependent destruction sas.
      generalize dependent saa.
      generalize (carve_sliceannshapedim_right_is_inc_sliceannshapedim_n_right_inverse (sascons s sas) axis 0 axis_lt_sdim
     sas_has_no_rpad).
      destruct s as [abeg aend pbeg pend].
      induction axis as [|axis'].
      (* axis = O *)
      * simpl. unfold carve_sliceanndimbound_right. simpl.
        generalize (carve_sliceanndimbound_right_obligation_2 bound (sadim bound abeg aend pbeg pend)). simpl.
        intro l. rewrite (proof_irrelevance _ _ (le_refl aend)). clear l.
        intro e. dependent destruction e. clear x.
        generalize dependent e0. rewrite (proof_irrelevance _ pend (Nat.le_refl aend)).
        intros e. rewrite (UIP_refl _ _ e). clear e. simpl.
        destruct saa as [v]. simpl. f_equal.
        generalize (take_obligation_1 dim (scons (sliceanndimbound (sadim aend abeg aend pbeg (Nat.le_refl aend))) (outershape sas))
     aend (Nat.le_refl aend)).
        generalize dependent v. simpl. unfold sliceanndimbound.
        intros v le_refl_. rewrite (proof_irrelevance _ le_refl_ (le_refl _)). clear le_refl_.
        induction v as [|dim' e v' IHv'].
          (* v = vnil *)
          + reflexivity...
          (* v = vcons e v' *)
          + simpl. f_equal. rewrite (proof_irrelevance _ (le_S_n dim' dim' (Nat.le_refl (S dim'))) (Nat.le_refl dim')).
            assumption.
      (* axis = S axis' *)
      * simpl. unfold transport. simpl. unfold eq_rect.
        generalize (@innershape_id_inc_sliceannshapedim_n (S dim)
                        (@sascons dim bound (sadim bound abeg aend pbeg pend) sas) (S axis') O axis_lt_sdim).
        simpl. intro e. rewrite (UIP_refl _ _ e). clear e. simpl.
        unfold inv_cpadr_all_obligation_7. unfold EqdepFacts.internal_eq_rew_r_dep. simpl.
        unfold DepElim.solution_left_dep. unfold apply_noConfusion. simpl.
        unfold DepElim.eq_symmetry_dep. simpl.
        destruct dim as [|dim'].
          (* dim = O *)
          + unfold False_rect.
            generalize (Nat.nlt_0_r axis' (lt_S_n axis' 0 axis_lt_sdim)).
            intro absurd. exfalso. apply absurd.
          (* dim = S dim' *)
          + simpl. unfold eq_rect_r. unfold eq_rect. intros e saa. simpl.
            generalize (proof_irrelevance (axis' < S dim') (Le.le_S_n (S axis') (S dim') axis_lt_sdim)
         (inv_cpadr_all_obligation_6 (S dim') (sascons (sadim bound abeg aend pbeg pend) sas) saa 
            (S axis') axis_lt_sdim pi_gt_O axis' (S dim') bound (sadim bound abeg aend pbeg pend) sas eq_refl eq_refl
            eq_refl)).
            unfold inv_cpadr_all_obligation_6.
            unfold eq_ind_r. unfold eq_ind. simpl. unfold apply_noConfusion. simpl.
            unfold DepElim.solution_left_dep. unfold DepElim.eq_symmetry_dep. simpl. simpl_eq.
            simpl in sas_has_no_rpad.
            dependent destruction sas_has_no_rpad.
            generalize dependent e. unfold elems.
            unfold sliceanndimbound. simpl.
            rewrite (proof_irrelevance _ (lt_S_n axis' (S dim') axis_lt_sdim) (Le.le_S_n (S axis') (S dim') axis_lt_sdim) ).
            intros e eq_refl_. rewrite (UIP_refl _ _ eq_refl_). clear eq_refl_.
            generalize dependent e. simpl.
            generalize (inv_cpadr_all_obligation_5 (S dim') (sascons (sadim bound abeg aend pbeg pend) sas) saa 
                   (S axis') axis_lt_sdim pi_gt_O axis' (S dim') bound (sadim bound abeg aend pbeg pend) sas eq_refl
                   eq_refl eq_refl). simpl.
            intro f_O_lt_pi_innershape. (* experiment! *)
            induction bound as [|bound'].
              (* bound = O, absurd *)
              ** exfalso. simpl in pi_gt_O. clear IHaxis'.
                 rewrite <- (le_n_0_eq _ pend) in pi_gt_O.
                 simpl in pi_gt_O.
                 apply (Nat.nlt_0_r _ pi_gt_O).
              ** simpl. Search (?x < 0). simpl. unfold eq_rect. simpl. unfold proj1_sig.
            destruct saa as [v]. (* TODO: stopped here *)
            generalize dependent v.
            unfold split_slices_saa.rewrite <- x.
            pattern (carve_sliceannshapedim_right sas axis').
            pattern (carve_sliceannshapedim_right sas axis' (Le.le_S_n (S axis') (S dim') axis_lt_sdim)).
            intro e. dependent destruction e.
            generalize dependent e0.
            rewrite <- x1.
            generalize dependent e0. simpl_one_JMeq. simpl. rewrite <- x.
            
            unfold carve_sliceannshapedim_right.
            generalize (carve_sliceannshapedim_right_obligation_2 (S (S dim')) (sascons (sadim bound abeg aend pbeg pend) sas)
         (S axis') axis_lt_sdim (S dim') bound (sadim bound abeg aend pbeg pend) sas eq_refl JMeq_refl axis' eq_refl).
            unfold carve_sliceannshapedim_right_obligation_2.
            generalize dependent e. simpl.
          apply absurd. simpl. Set Printing All. unfold eq_refl. simpl.
        unfold sigma_map. simpl. intros.
        unfold eq_trans.
        generalize length_l_is_n.
        
        generalize (inv_cpadr_all_obligation_4 dim (sascons (sadim bound abeg aend pbeg pend) sas) saa
                             (S axis') axis_lt_sdim pi_gt_O axis' dim bound (sadim bound abeg aend pbeg pend) sas
                             eq_refl eq_refl eq_refl).
        generalize (carve_sliceannshapedim_right_obligation_2 (S dim) (sascons (sadim bound abeg aend pbeg pend) sas) 
           (S axis') axis_lt_sdim dim bound (sadim bound abeg aend pbeg pend) sas eq_refl JMeq_refl axis' eq_refl).
        simpl. destruct saa as [v]. simpl.
        unfold inv_cpadr_all_obligation_7. simpl. simpl_eq. unfold eq_rect.

        unfold eq_rect. intros.
        
               (S axis') axis_lt_sdim dim bound (sadim bound abeg aend pbeg pend) sas eq_refl JMeq_refl axis' eq_refl). f_equal.
        dependent destruction v.
        destruct v.
        unfold take_vect.
        unfold elems.
        unfold elems.
        unfold carve_array_axis_right_obligation_2. simpl. generalize dependent x. simpl_JMeq.
      unfold inv_cpadr_all.
  intros. generalize dependent dim. generalize dependent n.
  induction axis as [|axis' IHaxis'].
    (* axis = O *)
    - intro n. induction n as [|n' IHn'].
      (* n = O *)
      * intros. simpl. unfold eq_rect.
        generalize dependent saa.
        generalize (carve_sliceannshapedim_right_is_inc_sliceannshapedim_n_right_inverse sas 0 0 axis_lt_sdim sas_has_no_rpad).
        generalize dependent pi_gt_O.
        generalize (@innershape_id_inc_sliceannshapedim_n (S dim) sas O O axis_lt_sdim). simpl.
        intro e. rewrite (UIP_refl _ _ e). simpl. clear e.
        dependent destruction sas.
        intros.
        generalize dependent e.
        generalize (inv_cpadr_all_obligation_2 dim (sascons s sas) saa 0 axis_lt_sdim pi_gt_O dim bound s sas eq_refl eq_refl eq_refl).
        simpl. intro e. rewrite (UIP_refl _ _ e). clear e.
        simpl in sas_has_no_rpad.
        destruct s as [abeg aend pbeg pend].
        unfold carve_sliceanndimbound_right. simpl.
        unfold sliceanndimend.
        unfold carve_sliceanndimbound_right in sas_has_no_rpad. simpl in sas_has_no_rpad.
        dependent destruction sas_has_no_rpad.
        rewrite (proof_irrelevance _ (carve_sliceanndimbound_right_obligation_2 aend (sadim aend abeg aend pbeg pend)) pend).
        intro e. rewrite (UIP_refl _ _ e). simpl. clear e.
        unfold carve_array_axis_right_obligation_2.
        unfold eq_rect_r. simpl.
        destruct saa. simpl.
        f_equal.
        generalize dependent v.
        generalize (take_obligation_1 dim (scons (sliceanndimbound (sadim aend abeg aend pbeg pend)) (outershape sas)) aend pend).
        simpl. unfold sliceanndimbound.
        generalize (aend * pi (outershape sas)).
        intros n le_refl_ v.
        rewrite (proof_irrelevance _ le_refl_ (le_refl n)).
        induction v as [|n' e v'].
          (* v = vnil *)
          + reflexivity...
          (* v = vcons e v' *)
          + simpl. rewrite (proof_irrelevance _ _ (le_refl n')).
            f_equal. apply (IHv' (le_refl n')).
      * intros. simpl. unfold eq_rect.
        generalize dependent saa.
        generalize (@transport (shape (S dim)) (@innershape (S dim) sas)
                (@innershape (S dim)
                   (@inc_sliceannshapedim (S dim)
                      (@inc_sliceannshapedim_n (S dim) sas O n'
                         (inc_sliceannshapedim_n_obligation_1 (S dim) O axis_lt_sdim)) O axis_lt_sdim))
                (fun x : shape (S dim) => lt O (@pi (S dim) x)) pi_gt_O
                (@eq_sym (shape (S dim))
                   (@innershape (S dim)
                      (@inc_sliceannshapedim (S dim)
                         (@inc_sliceannshapedim_n (S dim) sas O n'
                            (inc_sliceannshapedim_n_obligation_1 (S dim) O axis_lt_sdim)) O axis_lt_sdim))
                   (@innershape (S dim) sas)
                   (@innershape_id_inc_sliceannshapedim_n (S dim) sas O (S n') axis_lt_sdim))).
        generalize (cpadr_n_obligation_2 dim sas 0 axis_lt_sdim pi_gt_O n').
        destruct n' as [|n''].
          (* n' = O *)
          + simpl. intros e l. rewrite (UIP_refl _ _ e). clear e. simpl.
            generalize dependent pi_gt_O. simpl.
            generalize  (carve_sliceannshapedim_right_is_inc_sliceannshapedim_n_right_inverse sas 0 1 axis_lt_sdim sas_has_no_rpad).
            simpl. intro sas_has_no_rpad_inc.
            dependent destruction sas. simpl.
            destruct s as [abeg aend pbeg pend]. simpl.
            simpl_eq.
            clear l.
            unfold eq_ind. unfold sliceanndimbound. unfold eq_rect. intros saa pi_gt_O.
            generalize (cpadr_obligation_2 dim (sascons (sadim bound abeg aend pbeg pend) sas) pi_gt_O 0 axis_lt_sdim saa dim
               bound (sadim bound abeg aend pbeg pend) sas eq_refl eq_refl eq_refl).
            generalize dependent sas_has_no_rpad_inc.
            rewrite (proof_irrelevance _ axis_lt_sdim (Nat.lt_0_succ dim)).
            intros. rewrite (UIP_refl _ _ e). clear e.
            Set Printing All.
            destruct sas as [|sas'].
              ** simpl.
            generalize (match JMeq_eq JMeq_refl in (_ = y) return (bound = get_sliceanndimbound y 0 (Nat.lt_0_succ dim)) with
             | eq_refl => eq_refl
             end).
            generalize ( @JMeq_eq (sliceannshape (S dim)) (@sascons dim bound (sadim bound abeg aend pbeg pend) sas)
                    (@sascons dim bound (sadim bound abeg aend pbeg pend) sas)
                    (@JMeq_refl (sliceannshape (S dim)) (@sascons dim bound (sadim bound abeg aend pbeg pend) sas))). simpl. simpl. axis_lt_sdim).
            generalize (Nat.lt_0_succ dim). simpl. simpl. rect.
            unfold eq_rect. unfold eq_ind. unfold sliceanndimend. unfold inc_sliceanndimbound.
            unfold sliceanndimbound. simpl.
            pattern (JMeq_eq JMeq_refl) at 1.
            rewrite (proof_irrelevance _ (JMeq_eq JMeq_refl) eq_refl).
            rewrite (UIP_refl _ _ (JMeq_eq JMeq_refl)).
            unfold JMeq_eq. simpl_one_JMeq. All. generalize (JMeq_eq JMeq_refl). simpl. on_JMeq.
            generalize (eq_ind (sascons s sas)
                 (fun sas0 : sliceannshape (S dim) => bound = get_sliceanndimbound sas0 0 axis_lt_sdim) eq_refl
                 (sascons s sas) (JMeq_eq JMeq_refl)). rewrite <- sas_has_no_rpad_inc.
            generalize (inv_cpadr_all_obligation_2 dim (inc_sliceannshapedim sas 0 axis_lt_sdim)).
            rewrite (innershape_id_inc_sliceannshapedim sas 0 axis_lt_sdim).
 simpl.
        unfold inc_sliceannshapedim_n. simpl.
        
        generalize (carve_sliceannshapedim_right_is_inc_sliceannshapedim_n_right_inverse sas 0 
     (S n') axis_lt_sdim sas_has_no_rpad).
        simpl. dependent destruction sas. simpl.
        rewrite (innershape_id_inc_sliceannshapedim (inc_sliceannshapedim_n sas 0 n' (inc_sliceannshapedim_n_obligation_1 (S dim) 0 axis_lt_sdim)) 0 axis_lt_sdim).
        rewrite (innershape_id_inc_sliceannshapedim. unfold innershape. unfold inc_sliceannshapedim. unfold set_sliceanndim.
        unfold take_vect.
        generalize (take_obligation_1 dim
        (scons aend
           ((fix outershape (dim0 : nat) (sas0 : sliceannshape dim0) {struct sas0} : shape dim0 :=
               match sas0 in (sliceannshape n) return (shape n) with
               | sasnil => snil
               | @sascons dim1 bound _ subannshape => scons bound (outershape dim1 subannshape)
               end) dim sas)) aend pend). simpl.
        intro e. rewrite (proof_irrelevance _ e le_refl). le_refl).
        Set Printing All.
        fold (outershape (dim:=dim) sas).
        fold (outershape).
        unfold take_vect. simpl.
        unfold take. simpl.
        generalize (carve_sliceanndimbound_right_obligation_2 bound (sadim bound abeg aend pbeg pend)).
        simpl.
        unfold elems.
        generalize dependent saa. simpl.
        unfold transport.
        rewrite sas_has_no_rpad.
        unfold inv_cpadr_allO. unfold eq_rect.
        intros.
        unfold inv_cpadr_allO. simpl. unfold transport. simpl.
        generalize (Nat.lt_0_succ dim).
        generalize (carve_sliceannshapedim_right sas 0 (Nat.lt_0_succ dim)).
        generalize (inv_cpadr_allO saa pi_gt_O).
        rewrite (proof_irrelevance _ (Nat.lt_0_succ dim) axis_lt_sdim).
         simpl. intro. simpl.
        rewrite sas_has_no_rpad. simpl.
        generalize (carve_sliceannshapedim_right sas 0 axis_lt_sdim).
        rewrite <- sas_has_no_rpad.
        pattern (carve_sliceannshapedim_right sas 0 axis_lt_sdim) at 2.
        generalize (carve_sliceannshapedim_right (carve_sliceannshapedim_right sas 0 axis_lt_sdim) 0 axis_lt_sdim).
        pattern (carve_sliceannshapedim_right sas 0 axis_lt_sdim).
        pattern (carve_sliceannshapedim_right s 0 axis_lt_sdim
        unfold transport.
        pattern sas at 2. simpl.
simpl.  unfold inv_cpadr_all. unfold eq_rect.
      unfold ransport. simpl.
      generalize (inc_sliceannshapedim_n sas 0 n axis_lt_sdim). simpl.
      destruct (inc_sliceannshapedim_n sas 0 n axis_lt_sdim). simpl.
      generalize (inv_cpadr_all_obligation_2 dim (inc_sliceannshapedim_n sas 0 n axis_lt_sdim)
         (cpadr_n saa 0 n axis_lt_sdim pi_gt_O) 0 axis_lt_sdim
         (transport pi_gt_O (eq_sym (innershape_id_inc_sliceannshapedim_n 0 n axis_lt_sdim))) dim0 bound s s0 ddim
         daxis H).
      generalize (carve_sliceannshapedim_right_is_inc_sliceannshapedim_n_right_inverse sas axis 0 axis_lt_sdim sas_has_no_rpad).
      rewrite sas_has_no_rpad. unfold transport.  dependent destruction sas.
      destruct s as [abeg aend pbeg pend].
      simpl in sas_has_no_rpad.
      simpl.
      apply sas_has_no_rpad.
